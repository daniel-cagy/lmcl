...
pub fn parse_lines(input_lines: Vec<String>) -> Result<String, String>{
    ...
    process_lines_into_html(input_lines, &mut html_code, &mut stored_elements)?;
    html_code.push(String::from("</body>\n</html>"));
    Ok(html_code.join("\n"))
}

fn process_lines_into_html(input_lines: Vec<String>, html_code: &mut Vec<String>, stored_elements: &mut HashMap<String, String>) -> Result<(), String>{
    for (line_num, line) in input_lines.into_iter().enumerate(){
        if line.starts_with("let"){
            // Element will be stored and placed
            let (decl_type, decl_name, value) = declaration_tokens(line_num, &line, &stored_elements)?;
            if stored_elements.contains_key(decl_name){
                return Err(format!("Multiple elements with same name, on line {}: {}", line_num+1, line));
            }
            html_code.push(convert_declaration_to_html(decl_type, decl_name, &value, line_num, &line)?);
            stored_elements.insert(decl_name.to_string(), value.to_string());
        }
        else if line.starts_with("store"){
            // Element will be only stored
            let (store_name, value) = storing_tokens(line_num, &line, &stored_elements)?;
            ...
            let (place_type, value) = placement_tokens(line_num, &line, &stored_elements)?;
            html_code.push(convert_placement_to_html(place_type, &value, line_num, &line)?);
        }
    }
    Ok(())
}

fn split_line(line_num: usize, line: &str) -> Result<(&str, &str), String>{
...
fn resolve_value(right: &str, line_num: usize, line: &str, stored_elements: &HashMap<String, String>) -> Result<String, String>{
...
fn extract_tokens<'a>(expected_tokens: usize, line_num: usize, line: &'a str, stored_elements: &'a HashMap<String, String>) -> Result<(Vec<&'a str>, String), String>{
...
fn placement_tokens<'a>(line_num: usize, line: &'a str, stored_elements: &'a HashMap<String, String>) -> Result<(&'a str, String), String>{
    let (tokens, value) = extract_tokens(2, line_num, line, stored_elements)?;
    // Placements only take type and value
    Ok((tokens[1], value))
}

fn storing_tokens<'a>(line_num: usize, line: &'a str, stored_elements: &'a HashMap<String, String>) -> Result<(&'a str, String), String>{
    let (tokens, value) = extract_tokens(2, line_num, line, stored_elements)?;
    // Storings only take name and value
    Ok((tokens[1], value))
}

fn declaration_tokens<'a>(line_num: usize, line: &'a str, stored_elements: &'a HashMap<String, String>) -> Result<(&'a str, &'a str, String), String>{
...
fn convert_declaration_to_html(decl_type: &str, decl_name: &str, value: &str, line_num: usize, line: &str) -> Result<String, String>{
...
fn convert_placement_to_html(place_type: &str, value: &str, line_num: usize, line: &str) -> Result<String, String>{
...
fn analyze_class<'a>(input_type: &'a str, line_num: usize, line: &'a str) -> Result<(&'a str, &'a str), String>{
...
fn type_conversion(input_type: &str, line_num: usize, line: &str) -> Result<String, String>{
